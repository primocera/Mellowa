...
| MW-95-03 | `docs/beta-scorecard.md` predeclares D2/D3, plan_repair_previewed, preview→apply, Week opened, carry-forward, support burden, trial→charge, renewal, refund — several not computed in `report.ts`; no `plan_repair_previewed` event; `onboarding_completed` is client-describable | **preview metric RESOLVED (v19 MW-10)** — the nonexistent `plan_repair_previewed`/`preview→apply` removed from the active scorecard and replaced with `plan_repair_requested → plan_repair_completed`, `repeat_repair_distinct_day` and `plan_repair_undone`; `onboarding_completed` made server-authoritative in v17 (MW-V17-06) |
